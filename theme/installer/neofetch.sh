...
# Author info
echo "Jangan lupa folow akun 
tiktok @logic__vibes yawwww, thankss :)"

echo ""
echo "-------"

if ! command -v neofetch &> /dev/null; then
    echo -e "${C_RED}Error: neofetch not installed!${RST}"
    echo -e "${C_YELLOW}Install with: pkg install neofetch${RST}"
    exit 1
fi
...
if [ -f "$CONFIG_FILE" ]; then
    echo -e "${C_YELLOW}Backing up original neofetch config...${RST}"
    cp "$CONFIG_FILE" "$BACKUP_DIR/config.conf.backup.$(date +%Y%m%d_%H%M%S)"
    echo -e "${C_GREEN}✓ Backup saved to: $BACKUP_DIR/${RST}"
else
    echo -e "${C_YELLOW}No existing config found, creating fresh...${RST}"
    mkdir -p "$NEOFETCH_DIR"
fi

# Create the config file with Tux logo
cat > "$CONFIG_FILE" << 'EOF'
